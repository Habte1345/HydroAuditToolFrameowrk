...
def _setup_run(cfg: Dict) -> Dict:
    ...
    with (cfg["run_dir"] / 'cfg.json').open('w') as fp:
        temp_cfg = {}
        for key, val in cfg.items():
            if isinstance(val, (Path, PosixPath)):
...
def evaluate(user_cfg: Dict):
    ...
    for basin in tqdm(basins):
        ...
        # preds and obs are already 1-to-1 daily predictions → just flatten & align
        preds = preds.flatten()
        obs = obs.flatten()

        # Match date length
        preds = preds[:len(date_range)]
        obs = obs[:len(date_range)]

        df = pd.DataFrame({'qobs': obs, 'qsim': preds}, index=date_range)
...
def evaluate_basin(model: nn.Module, loader: DataLoader) -> Tuple[np.ndarray, np.ndarray]:
    ...
    with torch.no_grad():
        for data in loader:
            ...
            # Support both: no_static=True and static case
            if len(data) == 3:
                x, y, _ = data
            else:
                x, y = data
